...
1. ヒアリング: お客様のお悩み・背景を深掘りできているか
   ※ 聞いた項目数だけで高く評価しない。次に当てはまるときは★を1つ下げる(下限★1):
     ・お客様の答えに共感も相槌も返さないまま次の質問に移る場面が続く
     ・一問一答が続き、会話として流れていない
     ・聞いた内容を姿勢分析・整体・提案につなげず「聞いて終わり」になっている
   ※ 逆に、答えを受けて予定になかった深掘りができている/聞いた内容を提案に結びつけている
     場合は★を1つ上げてよい(上限★5)。★を上下させた理由はFB本文に一言書く
...
def _salvage_json(text: str) -> dict:
    """壊れたJSONから必要フィールドを正規表現で救済抽出する最終手段"""
    result = {}
    ...
    try:
        ...
        try:
            ...
            if salvaged.get("scores") or salvaged.get("session_summary"):
                salvaged["_warnings"] = ["JSON崩れのため一部フィールドを救済抽出しました"]
                return salvaged
            raise RuntimeError(f"Gemini JSON parse失敗(救済も失敗)\nresponse: {text[:300]}")
...
def evaluate_from_transcript(transcript: str, staff_name: str, session_date,
    ...
    text_prompt += (
        ...
        'scores / session_summary / good_points / improvements のみを出力してください。'
    )
        ...
        if dm_open.get("ok"):
            ...
            requests.post("https://slack.com/api/chat.postMessage", headers=H,
                          data=json.dumps({"channel": dm_id,
                                "text": f"✅ *育成FB処理完了*\n{staff_name} さん({session_date})の評価が #ピラティス_新規振り返り に投稿されました📩"}, ensure_ascii=False).encode("utf-8"))
