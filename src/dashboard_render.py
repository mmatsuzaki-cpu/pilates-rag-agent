...
<style>{_css(s)}</style></head><body>
  ...
  <div class="footer">契約率の色分け　緑=50%以上 / 橙=1〜49% / 赤=0% / 灰=母数なし{_prev_note(d)}</div>
</body></html>"""


def _prev_note(d):
    pd = d.get("prev_date")
    return f"　｜　前日比 = {pd} 比" if pd else ""
...
<style>{_css(s)}</style></head><body>
  ...
  <div class="grid">{cards}</div>
  <div class="footer">契約率の色分け　緑=50%以上 / 橙=1〜49% / 赤=0% / 灰=母数なし　|　数字 = 契約/新規{_prev_note(d)}</div>
</body></html>"""
