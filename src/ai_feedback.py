...
7. スコープ厳守: FBは「カウンセリング(ヒアリング)・クロージング(契約獲得)・接客対応」のみ。
   ピラティスの技術指導(ワーク選択・キューイング・エクササイズ内容・呼吸法の指導方法など)には
   一切触れない(技術系FBは研修担当が別途行う)。事例に技術的な内容があっても参考にしない。
8. お客様の年齢は、振り返りの「年齢層」欄に書かれた年代が正。年代を推測で書き換えない
   (記載なしのときは年代に触れない)。「既往歴」があれば提案時の注意点として考慮する。"""
